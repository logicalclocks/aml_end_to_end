...
class GANAnomalyModel(
    collections.namedtuple('GANAnomalyModel', (
        'generator_inputs',
        'generated_data',
        'generator_variables',
        'generator_scope',
  ...
  """A GANAnomalyModel contains all the pieces needed for GAN anomaly model training.
  ...
  Args:
    ...
    discriminator_real_outputs: The discriminator's output on real data.
    discriminator_gen_outputs: The discriminator's output on generated data.
    discriminator_predictions: The discriminator's output on generated data.
    discriminator_variables: A list of all discriminator variables.
    discriminator_scope: Variable scope all discriminator variables live in.
    discriminator_fn: The discriminator function.
  """

class GANEncoderAnomalyModel(
    collections.namedtuple('GANEncoderAnomalyModel', (
            'generator_inputs',
            'generated_data',
            'generator_variables',
            'generator_scope',
    ...
    # TODO: davit revise docstring
  """A GANEncoderAnomalyModel contains all the pieces needed for GAN anomaly model training.
...
class FAnoGANzModel(
    collections.namedtuple('FAnoGANAModel', (
    ...
    # TODO (davit): revise docstring
    """A FAnoGANAModel contains all the pieces needed for GAN anomaly model training.
...
class GANencLoss(
    collections.namedtuple('GANencLoss', (
        'generator_loss',
        'discriminator_loss',
        'encoder_loss'
    ))):
  # TODO (davit): rewrite
  """GANencLoss contains the generator and discriminator losses.
...
class FanoganZloss(
    collections.namedtuple('FanoganZloss', (
            'loss_data',
            'loss_fts',
            'z_distance',
            'loss'
    ...
    # TODO (davit): revise docstring
      ...
      """EncoderLoss contains the generator and discriminator losses.
...
class EncTrainOps(
    collections.namedtuple('EncTrainOps', (
        'encoder_train_op',
        'global_step_inc_op',
        'train_hooks'
    ))):
  """EncTrainOps contains the training ops.

  Args:
    encoder_train_op: Op that performs a encoder update step.
    train_hooks: a list or tuple containing hooks related to training that need
      to be populated when training ops are instantiated. Used primarily for
      sync hooks.
  """

  def __new__(cls, encoder_train_op,
              global_step_inc_op, train_hooks=()):
    return super(EncTrainOps, cls).__new__(cls, encoder_train_op,
                                                global_step_inc_op, train_hooks)
...
class EncTrainSteps(
    collections.namedtuple('EncTrainSteps', (
        'encoder_train_steps',
    ))):
  """Contains configuration for the Encoder Training.

  Args:
    encoder_train_steps: Number of generator steps to take in each GAN step.
  """

...
class ProgressiveGANEstimator(tf.estimator.Estimator):
  ...
  def __init__(self,
    ...
    def _model_fn(features, labels, mode, params):
      ...
      gan_model = get_gan_model(mode, model.generator_fn, model.discriminator_fn, real_data,
      ...
      if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        estimator_spec = get_train_estimator_spec(model, scaffold, gan_loss, model.gan_train_ops, get_hooks_fn)
    ...
    super(ProgressiveGANEstimator, self).__init__(
...
def get_gan_model(mode,
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    ...
    gan_model = make_prediction_gan_model(generator_inputs, generator_fn,
      ...
      # TODO (davit): replace this with build_model train_experiment_lib
      gan_model = None
...
# # TODO (davit): replace this with scaffold and define_train_ops in train_experiment_lib
...
def get_train_estimator_spec(model, scaffold, gan_loss, train_ops, get_hooks_fn):
  ...
  # train_ops = train_op_fn(gan_model, gan_loss, optimizers.gopt, optimizers.dopt, is_chief=is_chief)
  training_hooks = get_hooks_fn(train_ops)

  train_ops = model.gan_train_ops

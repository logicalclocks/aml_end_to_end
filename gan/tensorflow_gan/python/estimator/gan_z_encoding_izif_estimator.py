...
__all__ = [
    'GANzEncoderizifEstimator',
    'SummaryType'
]

Optimizers = collections.namedtuple('Optimizers', ['eopt'])
...
summary_type_map = {
    SummaryType.VARIABLES:
        tfgan_summaries.add_encoder_izif_model_summaries,
}


class GANzEncoderizifEstimator(tf.estimator.Estimator):
  ...
  ```python
      ...
      gan_estimator = tfgan.estimator.GANEstimator(
          ...
          encoder_loss_fn=tfgan.losses.wasserstein_generator_loss,
          encoder_optimizer=tf.train.AdamOptimizer(0.1, 0.5))
  ...
  """
    ...
    Args:
      ...
      get_hooks_fn: A function that takes a `GANTrainOps` tuple and returns a
        ...
        Defaults to `train.get_enc_sequential_train_hooks()`.
      ...
      params: Optional `dict` of hyperparameters.  Will receive what is passed
        ...
        `encodel_loss`, they will be passed to `encodel_loss` during training and
        evaluation.
    ...
    """
        _validate_input_args(encoder_loss_fn,
                             use_loss_summaries,
                             get_hooks_fn)
        optimizers = Optimizers(encoder_optimizer)



        vars_to_load = [i[0] for i in tf.train.list_variables(ckpt_dir)]
        assignment_map = {variable.op.name: variable for variable in tf.global_variables() if
                          variable.op.name in vars_to_load}
        train_vars = tf.compat.v1.train.init_from_checkpoint(ckpt_dir, assignment_map)

        #########################################################################################################
        # TODO (davit): maybe only gen and dis variables?
        #  https://www.tensorflow.org/api_docs/python/tf/estimator/WarmStartSettings
        #  https://github.com/google-research/bert/issues/719
        #  https://stackoverflow.com/questions/49846207/tensorflow-estimator-warm-start-from-and-model-dir?rq=1
        # Default WarmStart loads all variable names
        # but you can define with one to load. usingh var_regex bellow will load every variable
        # except INPUT_NAME and OPTIMIZER_NAME.
        ...
        warm_start_from = tf.compat.v1.estimator.WarmStartSettings(ckpt_to_initialize_from=ckpt_dir
                                            ,vars_to_warm_start=train_vars #var_regex
                                            )
        ...
        def _model_fn(features, labels, mode, params):
            """GANzEncoderizifEstimator model function."""
            if mode not in [
            ...
            # TODO (davit): need to add labels for eval summary
            ...
            gan_model = get_gan_model(mode, generator_fn, discriminator_fn,
                                      encoder_fn, real_data, add_summaries)

            # Make GANLoss, which encapsulates the losses.
            if mode in [tf.estimator.ModeKeys.TRAIN, tf.estimator.ModeKeys.EVAL]:
                #TODO (davit): fix extract_encodel_loss_args_from_params
                encodel_loss_kwargs = {} #extract_encodel_loss_args_from_params(params) or {}
                encodel_loss = tfgan_train.f_anogan_z_loss(
                    gan_model,
                    encoder_loss_fn,
                    **encodel_loss_kwargs)
            ...
            if mode == tf.estimator.ModeKeys.TRAIN:
                estimator_spec = get_train_estimator_spec(
                    gan_model, encodel_loss, optimizers, get_hooks_fn, is_chief=is_chief)

            elif mode == tf.estimator.ModeKeys.EVAL:
                estimator_spec = get_eval_estimator_spec(
                    gan_model, encodel_loss, get_eval_metric_ops_fn)
            else:  # tf.estimator.ModeKeys.PREDICT
                estimator_spec = get_predict_estimator_spec(gan_model, encodel_loss)

            return estimator_spec


        super(GANzEncoderizifEstimator, self).__init__(
            model_fn=_model_fn, model_dir=model_dir, config=config, params=params,
            warm_start_from=warm_start_from)


def _validate_input_args(encoder_loss_fn,
                         use_loss_summaries,
                         get_hooks_fn):
...
def get_gan_model(mode,
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        ...
        gan_model = make_prediction_gan_model(generator_fn, discriminator_fn, encoder_fn,
                                              real_data, generator_scope,
                                              discriminator_scope, encoder_scope)

    else:  # tf.estimator.ModeKeys.TRAIN or tf.estimator.ModeKeys.EVAL
        gan_model = _make_gan_model(generator_fn, discriminator_fn, encoder_fn, real_data,
                                    generator_scope, discriminator_scope, encoder_scope,
                                    add_summaries, mode)

    return gan_model


def _make_gan_model(generator_fn, discriminator_fn, encoder_fn, real_data,
                    generator_scope, discriminator_scope, encoder_scope,
                    add_summaries, mode):
    """Construct a `GANModel`, and optionally pass in `mode`."""
    # If network functions have an argument `mode`, pass mode to it.
    # TODO (davit): generator and dicrimator are not in training mode here, so please fix it
    ...
    gan_model = tfgan_train.z_encoding_izif_model(
        generator_fn,
        discriminator_fn,
        encoder_fn,
        real_data,
...
# TODO: (davit): work in progress
# TODO: (davit): here I think its a good place for anomaly scoring?
def make_prediction_gan_model(generator_fn, discriminator_fn, encoder_fn,
                              real_data, generator_scope,
                              discriminator_scope, encoder_scope):
    # FIXME (davit): decide here about real_data and labels
    ...
    # If `discriminator_fn` has an argument `mode`, pass mode to it.
    if 'mode' in inspect.getargspec(discriminator_fn).args:
        discriminator_fn = functools.partial(
            discriminator_fn, mode=tf.estimator.ModeKeys.PREDICT)
    # If `encoder_fn` has an argument `mode`, pass mode to it.
    if 'mode' in inspect.getargspec(encoder_fn).args:
        encoder_fn = functools.partial(
            encoder_fn, mode=tf.estimator.ModeKeys.PREDICT)

    with tf.compat.v1.variable_scope(encoder_scope) as enc_scope:
        encoded_data = encoder_fn(real_data)
    with tf.compat.v1.variable_scope(generator_scope) as gen_scope:
        generator_inputs = tfgan_train._convert_tensor_or_l_or_d(encoded_data)  # pylint:disable=protected-access
        recon_data = generator_fn(generator_inputs)
    with tf.compat.v1.variable_scope(discriminator_scope) as dis_scope:
        recon_features, _ = discriminator_fn(recon_data)
        real_features, _ = discriminator_fn(real_data)

    # TODO (davit): fix the variable scope here
    with tf.compat.v1.variable_scope(enc_scope):
        z_data_emb_query = encoder_fn(recon_data)

    encoder_variables = tf.compat.v1.get_collection(
        tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, enc_scope.name)
    ...
    discriminator_variables = tf.compat.v1.get_collection(
        tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, dis_scope.name)

    return tfgan_tuples.FAnoGANzModel(
...
def get_eval_estimator_spec(gan_model, encodel_loss, get_eval_metric_ops_fn=None):
    ...
    with tf.compat.v1.name_scope(name='metrics'):
        ...
        if get_eval_metric_ops_fn is not None:
            ...
            print(get_eval_metric_ops_fn)
            print("SSSSSSSSSSSSSSSSS")
            custom_eval_metric_ops = get_eval_metric_ops_fn(gan_model, encodel_loss)
    ...
    return tf.estimator.EstimatorSpec(
        ...
        predictions=gan_model.recon_data,
        loss=encodel_loss.loss,
        eval_metric_ops=eval_metric_ops)


def get_predict_estimator_spec(gan_model, encodel_loss):
    """Return an EstimatorSpec for the predict case."""

    predictions = {
            'emb_query': gan_model.encoded_data,
            'data_distance': encodel_loss.data_distance,
            'z_distance': encodel_loss.z_distance
        }
...
def _maybe_construct_optimizers(optimizers):
    ...
    return Optimizers(eopt)


def get_train_estimator_spec(
        gan_model, encodel_loss, optimizers,
        get_hooks_fn, train_op_fn=tfgan_train.encoder_train_ops, is_chief=True):
    """Return an EstimatorSpec for the train case."""
    get_hooks_fn = get_hooks_fn or tfgan_train.get_enc_sequential_train_hooks()
    optimizers = _maybe_construct_optimizers(optimizers)

    train_ops = train_op_fn(gan_model, encodel_loss, optimizers.eopt,
                            is_chief=is_chief)
...
# TODO (davit): please adjust this to encoder
def extract_encodel_loss_args_from_params(params):
    """Returns a dictionary with values for `encodel_loss`."""
    encodel_loss_arg_names = inspect.getargspec(tfgan_train.f_anogan_z_loss).args
    ...
    for forbidden_symbol in [
        ...
        encodel_loss_arg_names.remove(forbidden_symbol)
        if forbidden_symbol in params:
            raise ValueError('`%s` is not allowed in params.')

    encodel_loss_args = {k: params[k] for k in encodel_loss_arg_names if k in params}

    return encodel_loss_args

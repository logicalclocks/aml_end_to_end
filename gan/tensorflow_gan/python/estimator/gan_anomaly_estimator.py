...
class GANAnomalyEstimator(tf.estimator.Estimator):
  ...
  """
    ...
    """
        ...
        super(GANAnomalyEstimator, self).__init__(
...
def get_gan_model(mode,
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        ...
        gan_model = make_prediction_gan_model(generator_fn, discriminator_fn, real_data,
...
def _make_gan_model(generator_fn, discriminator_fn, real_data, generator_inputs,
    ...
    gan_model = tfgan_train.gan_anomaly_model(
        generator_fn,
        discriminator_fn,
        real_data,
        generator_inputs,
...
def make_prediction_gan_model(generator_fn, discriminator_fn, real_data,
                              generator_inputs, generator_scope,
                              discriminator_scope):
    # FIXME (davit): decide here about real_data and labels
    ...
    with tf.compat.v1.variable_scope(discriminator_scope) as dis_scope:
        _, ano_label_predictions = discriminator_fn(real_data)
    ...
    return tfgan_tuples.GANAnomalyModel(
        generator_inputs,
        generated_data,
        generator_variables,
        gen_scope,
        ...
        discriminator_predictions=ano_label_predictions,

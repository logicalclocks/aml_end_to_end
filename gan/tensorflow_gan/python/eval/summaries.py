...
__all__ = [
    ...
    'add_gan_anomaly_model_summaries',
...
def add_encoder_izif_model_summaries(encoder_izif_model):
    """Adds typical EncoderizifModel summaries.

    Args:
      encoder_izif_model: A EncoderizifModel tuple.
    """

    with tf.compat.v1.name_scope('encoder_variables'):
        for var in encoder_izif_model.encoder_variables:
            tf.compat.v1.summary.histogram(var.name, var)


def add_gan_anomaly_model_summaries(gan_model):
  #TODO (davit):
  """Adds typical GANAnomalyModel summaries.
  ...
  with tf.compat.v1.name_scope('generator_variables'):
    for var in gan_model.generator_variables:
      tf.compat.v1.summary.histogram(var.name, var)
    tf.compat.v1.summary.histogram("generated_data/histogram", gan_model.generated_data)
  with tf.compat.v1.name_scope('discriminator_variables'):
    for var in gan_model.discriminator_variables:
      tf.compat.v1.summary.histogram(var.name, var)
    tf.compat.v1.summary.histogram("real_data/histogram", gan_model.real_data)

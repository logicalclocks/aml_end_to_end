...
def generator_encoder_loss(
    generated_data, #gen_outputs
    generator_reconstracted_data, #gen_rec_outputs
    weights=1.0,
    scope=None,
    loss_collection=tf.compat.v1.GraphKeys.LOSSES,
    reduction=tf.compat.v1.losses.Reduction.SUM_BY_NONZERO_WEIGHTS,
  ...
  See `Wasserstein GAN` (https://arxiv.org/abs/1701.07875) for more details.
  See https://arxiv.org/abs/1905.11034 for more details
  ...
  with tf.compat.v1.name_scope(scope, 'generator_encoder_loss',
                               (generator_reconstracted_data, weights)) as scope:
    generator_reconstracted_data = _to_float(generator_reconstracted_data)


    # TODO (davit): evaluate which loss function is better here
    """
    gen_rec_loss = tf.compat.v1.losses.mean_squared_error(
        labels=generated_data,
        predictions=generator_reconstracted_data,
        weights=weights,
        scope=scope,
    ...
    # gen_rec_loss = tf.compat.v1.norm(
    #     tensor= generated_data - generator_reconstracted_data,
    #     ord='euclidean',
    #     axis=[-2,-1],
    #     keepdims=False,
    #     name=None,
    ...
    loss = tf.reduce_sum(tf.pow(generated_data - generator_reconstracted_data, 2), axis=[-2, -1])

    # TODO (davit): please remove this also from train gan_enc_anomaly_model
    # encoded_g_input = tf.compat.v1.get_default_graph().get_tensor_by_name("Encoder/encoded_origing_scope/Encoder/layer_2/fc2/BiasAdd:0")
    # z_loss = tf.reduce_sum(tf.pow(g_input - encoded_g_input, 2), axis=[-2, -1])
    # loss = loss + z_loss

    beta_cycle_gen = 10.0 #TODO (davit): check the effect of this parameter
    loss = loss * beta_cycle_gen
    loss = tf.compat.v1.losses.compute_weighted_loss(loss, weights, scope,
                                                     loss_collection, reduction)

    if add_summaries:
      # tf.compat.v1.summary.scalar('gen_latent_enc_loss', z_loss)
      tf.compat.v1.summary.scalar('gen_rec_loss', loss)
  ...
  with tf.compat.v1.name_scope(scope, 'wasserstein_gradient_penalty',
    ...
    with tf.compat.v1.name_scope(
      ...
      with tf.compat.v1.variable_scope(
          ...
          # TODO (davit): our discriminator_fn takes only 1 input and we only need interpolates. But this implementation
          #  will fail on original image gan implementations. So please have condition or modify
